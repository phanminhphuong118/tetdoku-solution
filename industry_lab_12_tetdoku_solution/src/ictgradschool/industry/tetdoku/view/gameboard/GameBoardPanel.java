...
import ictgradschool.industry.tetdoku.model.shapes.ShapeFactory;
...
public class GameBoardPanel extends JPanel {
    ...
    /**
     * The coordinates of the gameboard cell we're currently mousing over, if any.
     */
    private CellCoordinate currentMouseCellCoords;
    ...
    public GameBoardPanel() {
        ...
        this.shapeToPlace = ShapeFactory.createRandomShape();
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Draw the current shape at the user's current mouse location, if any
        if (this.shapeToPlace != null && this.currentMouseCellCoords != null) {
            GameBoardShapePainter shapePainter = new GameBoardShapePainter(
                    this.shapeToPlace, this.gameBoard,
                    getCurrentShapeCoords(this.currentMouseCellCoords), graphics, cellSize);
            shapePainter.paint();
        }
            ...
            if (shapeToPlace != null) {
                ...
                if (mouseCellCoordinates != null) {
                    ...
                    if (gameBoard.shapeFits(shapeToPlace, shapeCoordinates)) {
                        ...
                        // Change the shape to place
                        shapeToPlace = ShapeFactory.createRandomShape();
    ...
    /**
     * A {@link MouseMotionListener} lets us respond to the mouse moving around on the screen, amongst other things.
     * <p>
     * Whenever the mouse moves, figure out which gameboard cell the mouse is pointing to, if any. If that cell is
     * different than the one we currently have recorded, then updated it and force a repaint so the panel redraws
     * itself.
     */
    private final MouseMotionListener mouseMotionListener = new MouseMotionAdapter() {
        @Override
        public void mouseMoved(MouseEvent e) {
            CellCoordinate newMouseCellCoordinates = getCellCoordinates(e.getPoint());
            // Use of Objects.equals() prevents us from having to check for nulls ourselves.
            if (!Objects.equals(currentMouseCellCoords, newMouseCellCoordinates)) {
                currentMouseCellCoords = newMouseCellCoordinates;
                repaint();
            }
        }
    };
    ...
    /**
     * A mouse wheel listener that will cause the shape to mutate, then repaint.
     */
